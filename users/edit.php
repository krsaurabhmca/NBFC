...
$branches = mysqli_query($conn, "SELECT id, branch_name FROM branches WHERE status = 'active' ORDER BY branch_name ASC");
...
if($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['edit_user'])) {
    ...
    if(mysqli_num_rows($check) > 0) {
        ...
        $sql = "UPDATE users SET name = '$name', username = '$username', role = '$role', branch_id = $branch_id WHERE id = $id";
        ...
        if(!empty($password)) {
            if(strlen($password) < 6) {
                ...
                $sql = "UPDATE users SET name = '$name', username = '$username', password = '$hash', role = '$role', branch_id = $branch_id WHERE id = $id";
            }
        }
    ...
    <form method="POST" action="" class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 pt-2">
            ...
            <div>
                ...
                <select name="branch_id" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none flex-1">
                    <?php while($b = mysqli_fetch_assoc($branches)): ?>
                        <option value="<?= $b['id'] ?>" <?= $user['branch_id'] == $b['id'] ? 'selected' : '' ?>><?= htmlspecialchars($b['branch_name']) ?></option>
                    <?php endwhile; ?>
                </select>
            </div>
