...
$txn_id_str = isset($_GET['txn_id']) ? sanitize($conn, $_GET['txn_id']) : '';
...
if($txn_id_str) {
    ...
    // 6. Delete the transaction (soft delete / void is better, but here we delete for clean ledger)
    mysqli_query($conn, "DELETE FROM transactions WHERE id = " . $txn['id']);
