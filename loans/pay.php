...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['pay_emi'])) {
    ...
    if (empty($selected_schedules)) {
        ...
        if ($total_collected > 0) {
            ...
            if ($total_fine > 0)
                $desc .= " (Includes " . formatCurrency($total_fine) . " late fine)";
...
<div class="max-w-6xl mx-auto">
    <div class="mb-4 flex items-center justify-between">
        <div>
            <h1 class="text-xl font-bold text-gray-800">EMI Payment Center</h1>
            <p class="text-gray-500 text-xs">Collect loan money, give discounts, and clear late dues.</p>
        </div>
        <form action="" method="GET" class="flex gap-2">
            <select name="account_id" class="select2-init w-64 border rounded-lg px-3 py-2 text-sm"
            ...
            <div class="grid grid-cols-1 lg:grid-cols-4 gap-6">
                ...
                <div class="lg:col-span-3">
                    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                        <div class="p-4 bg-gray-50 border-b border-gray-100 flex justify-between items-center">
                            <h3 class="font-bold text-sm uppercase tracking-wider text-gray-600">Payment Plan & Late Dues List</h3>
                            ...
                            <table class="w-full text-sm">
                                <thead class="bg-gray-50 text-gray-500 text-left font-bold border-b border-gray-100">
                                    <tr>
                                        ...
                                        <th class="px-4 py-2 text-right">EMI Amount</th>
                                        <th class="px-4 py-2 text-right">Late Fine</th>
                                ...
                                <tbody class="divide-y divide-gray-50">
                                    <?php foreach ($schedules as $s): ?>
                                        <tr
                                            class="<?= $s['status'] == 'Overdue' ? 'bg-red-50/30' : ($s['status'] == 'Paid' ? 'bg-emerald-50/10' : '') ?>">
                                            <td class="px-4 py-2 text-center">
                                                <?php if ($s['status'] != 'Paid'): ?>
                                                    <input type="checkbox" name="schedules[]" value="<?= $s['id'] ?>"
                                                        data-emi="<?= $s['emi_amount'] ?>" data-fine="<?= $s['fine_amount'] ?>"
                                            ...
                                            <td class="px-4 py-2">
                                                ...
                                                <div class="text-[10px] text-gray-400 capitalize">
                                                    <?= date('l', strtotime($s['due_date'])) ?>
                                                </div>
                                            </td>
                                            <td class="px-4 py-2 text-right font-medium"><?= formatCurrency($s['emi_amount']) ?>
                                            </td>
                                            ...
                                            <td class="px-4 py-2 text-center">
                                                ...
                                                if ($s['status'] == 'Paid')
                                                    $status_cls = 'bg-emerald-100 text-emerald-700';
                                                if ($s['status'] == 'Overdue')
                                                    $status_cls = 'bg-rose-100 text-rose-700';
                                                if ($s['status'] == 'Pending')
                                                    $status_cls = 'bg-amber-100 text-amber-700';
                                                ?>
                                                <span
                                                    class="px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-widest whitespace-nowrap <?= $status_cls ?>">
                                                    <?= $s['status'] ?>
                                                </span>
                                                <?php if ($s['status'] == 'Paid'): ?>
                ...
                <div class="lg:col-span-1 space-y-4">
                    <div class="bg-indigo-900 rounded-xl p-5 text-white shadow-xl">
                        ...
                        <div class="space-y-3">
                            ...
                            <div class="pt-3 border-t border-indigo-800">
                                <span class="font-bold text-light-800 uppercase mb-2" style="color:#fff">Discount / Waiver
                                    (₹)</label>
                                    <input type="number" step="0.01" name="total_discount" id="discountInput" value="0.00"
                                        class="w-full bg-white/10 border border-white/20 rounded-lg px-3 py-2 text-white outline-none focus:ring-2 focus:ring-indigo-400">
                            </div>
                    ...
                    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 space-y-4">
                        <div>
                            <label
                                class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1.5">Payment
                                Mode</label>
                            <select name="payment_mode"
                                class="w-full border rounded-lg px-3 py-2 text-sm bg-gray-50 focus:bg-white outline-none">
                        ...
                        <div>
                            <label
                                class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1.5">Received
                                By</label>
                            <select name="received_by"
                                class="w-full border rounded-lg px-3 py-2 text-sm bg-gray-50 focus:bg-white outline-none">
                                <?php mysqli_data_seek($employees, 0);
                                while ($e = mysqli_fetch_assoc($employees)): ?>
                                    <option value="<?= $e['id'] ?>" <?= $e['id'] == $_SESSION['user_id'] ? 'selected' : '' ?>>
                        ...
                        <button type="submit" name="pay_emi" id="payBtn" disabled
                            class="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-3 rounded-xl shadow-lg shadow-emerald-50 transition-all flex items-center justify-center gap-2 disabled:opacity-50 disabled:grayscale">
    ...
    <?php else: ?>
        <div
            class="bg-white rounded-lg p-16 border border-gray-100 shadow-sm border-dashed flex flex-col items-center justify-center text-center">
            ...
            <p class="text-gray-500 text-sm max-w-sm mt-1">Please use the account selector at the top right to start
