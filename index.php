...
// 1. Portfolio Stats
$total_members = mysqli_fetch_assoc(mysqli_query($conn, "SELECT COUNT(*) as c FROM members WHERE 1=1 " . getBranchWhere('', false)))['c'];
$active_loans = mysqli_fetch_assoc(mysqli_query($conn, "SELECT COUNT(*) as c FROM accounts WHERE 1=1 " . getBranchWhere('', false) . " AND status = 'active' AND account_type = 'Loan'"))['c'];
$market_capital = mysqli_fetch_assoc(mysqli_query($conn, "SELECT SUM(principal_amount) as s FROM accounts WHERE 1=1 " . getBranchWhere('', false) . " AND status IN ('active','defaulted') AND account_type = 'Loan'"))['s'] ?? 0;
$pending_approvals = mysqli_fetch_assoc(mysqli_query($conn, "SELECT COUNT(*) as c FROM accounts WHERE 1=1 " . getBranchWhere('', false) . " AND status = 'pending_approval'"))['c'];
...
// 3. Recent Activity
$txn_where = " WHERE $date_where" . getBranchWhere('a', false);
...
<div class="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-4">
    <div>
        <h1 class="text-3xl font-black text-slate-800 tracking-tight">Loan System Overview</h1>
        <p class="text-slate-500 font-medium">Monitoring money and daily collections.</p>
    </div>
...
<div class="mb-8 bg-amber-50 border border-amber-200 p-5 rounded-xl flex items-center justify-between shadow-sm shadow-amber-100">
    <div class="flex items-center gap-4">
        <div class="w-12 h-12 bg-amber-500 text-white rounded-lg flex items-center justify-center shadow-lg shadow-amber-200">
            <i class="ph ph-shield-warning text-2xl font-black"></i>
        </div>
        <div>
            <h3 class="font-black text-amber-900 text-sm uppercase tracking-widest">New Loan Approval List</h3>
...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
    <div class="bg-white p-6 rounded-xl shadow-sm border border-slate-100 group relative overflow-hidden">
        <div class="relative z-10">
            <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Total Loan Issued</p>
            <h3 class="text-2xl font-black text-slate-800 tracking-tight"><?= formatCurrency($market_capital) ?></h3>
            <div class="mt-4 flex items-center gap-2">
                <span class="bg-emerald-100 text-emerald-700 text-[9px] font-black px-2 py-0.5 rounded-full uppercase">Main Loan Book</span>
            </div>
        </div>
    ...
    <div class="bg-indigo-600 p-6 rounded-xl shadow-xl shadow-indigo-100 text-white group relative overflow-hidden">
        <div class="relative z-10">
            <p class="text-[10px] font-black text-white/60 uppercase tracking-widest mb-1">Expected Money</p>
    ...
    <div class="bg-white p-6 rounded-xl shadow-sm border border-slate-100 group relative overflow-hidden">
        <div class="relative z-10">
            <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Money Collected</p>
    ...
    <div class="bg-white p-6 rounded-xl shadow-sm border border-slate-100 group relative overflow-hidden">
        <div class="relative z-10">
            <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Late Accounts (PBP)</p>
...
<div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
    ...
    <div class="lg:col-span-8 space-y-6">
        <div class="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden">
            <div class="px-6 py-5 border-b border-slate-50 flex items-center justify-between">
                <h3 class="font-black text-slate-800 text-xs uppercase tracking-widest">Recent Money Movements</h3>
    ...
    <div class="lg:col-span-4 space-y-6">
        <div class="bg-slate-900 rounded-xl p-6 text-white shadow-xl shadow-slate-200 border-b-8 border-b-indigo-600">
            <h3 class="text-[10px] font-black uppercase tracking-widest text-slate-500 mb-6">Area Collection Summary</h3>
            <div class="space-y-6">
                <div>
                    <div class="flex justify-between text-[10px] font-black uppercase tracking-widest mb-2 opacity-60">
                        <span>Total Money Issued</span>
                ...
                <div>
                    <div class="flex justify-between text-[10px] font-black uppercase tracking-widest mb-2 opacity-60">
                        <span>Main Money Collected</span>
                        <?php 
        ...
        <div class="bg-white rounded-xl p-6 border border-slate-100 shadow-sm">
            ...
            <div class="grid grid-cols-2 gap-3">
                <a href="loans/disburse.php" class="p-4 bg-indigo-50 text-indigo-600 rounded-lg flex flex-col items-center justify-center gap-2 hover:bg-indigo-600 hover:text-white transition-all group">
                ...
                <a href="loans/pay.php" class="p-4 bg-emerald-50 text-emerald-600 rounded-lg flex flex-col items-center justify-center gap-2 hover:bg-emerald-600 hover:text-white transition-all group">
